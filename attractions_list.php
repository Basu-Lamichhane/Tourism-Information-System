...
if (isset($_GET['destination']) && (!isset($_GET['district'])) && (!isset($_GET['type'])) && (!isset($_GET['query']))) { //
    ...
    $first_char = substr($destination, 0, 1);
    $records = $con->query("SELECT * FROM tbl_" . $destination . " INNER JOIN tbl_district ON " . $first_char . "_district=d_id where d_name='" . $district . "';");
    $no_of_rows = $records->num_rows;
    ...
    $district = $_GET['district'];
    $records = $con->query("
    SELECT id, name, address, district, type, source_table
    FROM (
        SELECT d_id AS id, d_name AS name, 'Nepal' AS address, d_name AS district, d_type AS type, 'tbl_district' AS source_table
        FROM tbl_district
        UNION
        SELECT p_id AS id, p_name AS name, p_address AS address, p_district AS district, p_type AS type, 'tbl_place' AS source_table
        FROM tbl_place
    ...
    WHERE name LIKE 'k%' AND d_name='Kathmandu'
    ORDER BY name ASC;
    ");
    $no_of_rows = $records->num_rows;
    $title_text = "Search Results for '" . $_GET['query'] . "'";
